...
export default function UpdateLocation() {
  ...
  // Fetch existing location data - enhanced for better compatibility
  useEffect(() => {
    if (!id) return;
    const controller = new AbortController();
        ...
        try {
          ...
          if (res.ok) {
            ...
            if (data) {
              ...
              const fetchedName = data.name || data.Name || data.locationName || data.LocationName || '';
              if (fetchedName) {
                setName(fetchedName);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        // Check for duplicate but exclude current location
        const match = list.find((x) => {
          const matchId = String(x?.id || x?.ID || x?.locationId || x?.LocationId || '');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payloads = [
        { id, Name: nameTrimmed },
        { id, name: nameTrimmed },
        { locationId: id, locationName: nameTrimmed },
        { LocationId: id, LocationName: nameTrimmed },
      ];
      ...
      >
        ...
        <div className="bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 px-8 py-6">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h2 className="text-3xl font-bold text-white">Update Location</h2>
          </div>
          <p className="text-indigo-100 text-sm mt-1">Modify location information</p>
        </div>
            ...
            {success && (
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-green-700 text-sm font-medium">Location updated successfully!</span>
                </div>
              </motion.div>
            )}
