...
export default function AddOwnerType() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error("Failed to add owner type");
      setSuccess(true);
      handleReset();
      setTimeout(() => router.push("/owner-type/list"), 1200);
    } catch (err: any) {
      setError(err.message || "Failed to add owner type");
    }
  };
        ...
        <header className="flex flex-col gap-4 rounded-3xl border border-slate-800 bg-gradient-to-br from-indigo-700/40 via-indigo-500/20 to-transparent p-6 shadow-xl shadow-indigo-900/40 backdrop-blur">
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <h1 className="mt-2 text-3xl font-semibold text-white">Create Owner Type</h1>
              <p className="mt-1 text-sm text-slate-300">
                Define how assets are owned or managed across teams, vendors, and partners.
              </p>
            </div>
        ...
        <section className="rounded-3xl border border-slate-800 bg-slate-900/70 p-6 shadow-2xl shadow-indigo-900/30 backdrop-blur">
          <form onSubmit={handleSubmit} autoComplete="off" className="space-y-6">
            <div className="grid gap-5 md:grid-cols-2">
              <div className="md:col-span-2 space-y-2">
                ...
                <p className="text-xs text-slate-500">
                  Choose a short, recognizable title that appears in asset records and filters.
                </p>
              ...
              <div className="md:col-span-2 space-y-2">
                ...
                />
                <p className="text-xs text-slate-500">
                  Explain the scope of this owner type to guide procurement and auditing teams.
                </p>
                {fieldErrors.description && (
            ...
            {success && (
              ...
              >
                Owner type added successfully! Redirecting to directory…
              </motion.div>
            )}
