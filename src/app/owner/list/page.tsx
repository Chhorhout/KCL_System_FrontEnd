...
export default function OwnerList() {
  ...
  useEffect(() => {
    ...
    fetchOwners(page, pageSize, controller.signal);
    return () => controller.abort();
  }, [page, pageSize]);

  const fetchOwners = async (pageNum: number, size: number, signal?: AbortSignal) => {
    setLoading(true);
    try {
      const url = new URL("http://localhost:5092/api/Owner");
      ...
      const normalized: Owner[] = Array.isArray(data)
        ? data
      ...
      if (normalized.length > 0) {
        console.log('[Owner List] Sample owner data:', normalized[0]);
      }

      // Fetch owner type names for each owner
      // First, try to get owner type ID from various possible property names
      const ownersWithTypes = await Promise.all(
        normalized.map(async (item: any) => {
          let ownerTypeName = null;
          // Check if owner type name is already included in the response
          if (item.ownerTypeName || item.ownerType?.name) {
            ownerTypeName = item.ownerTypeName || item.ownerType?.name;
          } else {
            // Try to get owner type ID from various possible property names
            const ownerTypeId = item.ownertypeId || item.ownerTypeId || item.ownerTypeID || item.OwnerTypeId;
            if (ownerTypeId) {
              try {
                const typeRes = await fetch(`http://localhost:5092/api/OwnerType/${ownerTypeId}`, { signal });
                if (typeRes.ok) {
                  const typeData = await typeRes.json();
                  ownerTypeName = typeData.name || null;
                  console.log(`[Owner List] Fetched owner type for ${item.name}: ${ownerTypeName}`);
                } else {
                  console.warn(`[Owner List] Failed to fetch owner type ${ownerTypeId} for owner ${item.name}: ${typeRes.status} ${typeRes.statusText}`);
                }
              } catch (e: any) {
                console.warn(`[Owner List] Error fetching owner type ${ownerTypeId} for owner ${item.name}:`, e?.message || e);
              }
            } else {
              console.warn(`[Owner List] No owner type ID found for owner ${item.name}. Available properties:`, Object.keys(item));
            }
          }
          return {
            id: item.id,
            name: item.name ?? "Unnamed",
            ownertypeId: item.ownertypeId ?? item.ownerTypeId ?? item.ownerTypeID ?? item.OwnerTypeId ?? "",
            ownerTypeName,
          };
        })
      );
  ...
  const handleDelete = async (id: string) => {
    ...
    if (result.isConfirmed) {
      try {
        const res = await fetch(`http://localhost:5092/api/Owner/${id}`, {
          method: "DELETE",
        });
        if (!res.ok) throw new Error("Failed to delete owner");
        Swal.fire("Deleted!", "The owner has been removed.", "success");
        fetchOwners(page, pageSize);
      } catch (err: any) {
        Swal.fire("Error", err.message || "Failed to delete owner", "error");
      }
    }
  };
  ...
  const filteredOwners = owners.filter((owner) => {
    ...
    return (
      ...
      (owner.ownerTypeName ?? "").toLowerCase().includes(term)
    );
  });

  const totalOwners = totalCount || owners.length;
  const hasOwners = totalOwners > 0;
  const startIndex = hasOwners ? (page - 1) * pageSize + 1 : 0;
  const endIndex = hasOwners
    ? Math.min(startIndex + filteredOwners.length - 1, totalOwners)
    : 0;
      ...
      >
        <header className="flex flex-col gap-4 rounded-2xl border border-slate-800 bg-slate-900/80 p-6 shadow-xl shadow-indigo-950/40 backdrop-blur md:flex-row md:items-center md:justify-between">
          <div className="space-y-2">
            ...
            <h1 className="text-3xl font-semibold text-white">Owners</h1>
            <p className="text-sm text-slate-300">Manage asset owners and their classifications.</p>
          </div>
          <Link href="/owner/add">
              ...
              <tbody className="divide-y divide-slate-800 bg-slate-900/60">
                {loading ? (
                  ...
                  filteredOwners.map((owner) => (
                    <tr key={owner.id} className="hover:bg-indigo-500/10">
                      ...
                      <td className="px-6 py-4 text-sm text-slate-200">
                        {owner.ownerTypeName || "No type assigned"}
                      </td>
                      <td className="px-6 py-4 text-right">
                        <div className="inline-flex items-center gap-2">
                          <Link href={`/owner/edit/${owner.id}`}>
        ...
        <section className="rounded-2xl border border-slate-800 bg-slate-900/70 p-4 shadow-xl shadow-indigo-950/30">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <p className="text-sm text-slate-300">
              {hasOwners
                ? `Showing ${startIndex} to ${endIndex} of ${totalOwners} owners`
